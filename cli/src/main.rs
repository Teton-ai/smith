    ...
    match cli.command {
        Some(Commands::Interactive) | None => {
            if io::stdin().is_terminal() {
                interactive::run_repl(&mut config).await?;
            } else {
                Cli::command().print_help()?;
            }
        }
        Some(command) => {
            dispatch_command(command, &mut config).await?;
        }
    }
...
pub async fn dispatch_command(
    ...
    match command {
        Commands::Interactive => {
            // When called from the REPL, this is handled before dispatch.
            // When called from the CLI directly, main() handles it.
            // This arm should not be reached, but handle gracefully.
            println!("Use 'sm' with no arguments to enter interactive mode.");
        }
        ...
        Commands::Get { resource } => match resource {
                ...
                if devices.len() == 1 {
                    ...
                    let status_str = if let Some(last_seen) = &device.last_seen {
                        ...
                        if duration.num_minutes() < 5 {
                            ...
                            let human_time = HumanTime::from(last_seen.with_timezone(&chrono::Utc));
            ...
            GetResourceType::Commands {
                ...
                for (idx, device) in devices.iter().enumerate() {
                    ...
                    let mut table =
                        TablePrint::new_with_headers(vec!["ID", "Issued At", "Command", "Status"]);
                    ...
                    for cmd in commands {
                        ...
                        table.add_row(vec![cmd.cmd_id.to_string(), issued_at, cmd_type, status]);
                    }
        ...
        Commands::Status { resource } => match resource {
            StatusResourceType::Device { selector, nowait } => {
                ...
                let device =
                    resolve_single_device_from_selector(&api, &selector, "status device").await?;
            ...
            StatusResourceType::Service {
                ...
                let device =
                    resolve_single_device_from_selector(&api, &selector, "status service").await?;
                let serial_number = device.serial_number.clone();
        ...
        Commands::Logs { selector, nowait } => {
            ...
            let logs = execute_device_command(&api, serial_number, "Fetching logs", nowait, |id| {
        ...
        Commands::Distributions { command } => match command {
            DistroCommands::Ls { json } => {
                ...
                let mut table = TablePrint::new_with_headers(vec!["Name (arch)", "Description"]);
                for d in distros {
                    table.add_row(vec![
        ...
        cli::Commands::Tunnel {
            ...
            if device.last_seen.is_none_or(|last_seen| {
                ...
                if let Some(last_seen) = device.last_seen {
                    last_ping = HumanTime::from(last_seen.with_timezone(&chrono::Utc)).to_string();
                }
        ...
        Commands::Releases { command } => {
            command.handle(config.clone()).await?;
        }
