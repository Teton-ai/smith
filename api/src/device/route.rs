...
#[utoipa::path(
    ...
    responses(
        ...
        (status = StatusCode::CONFLICT, description = "SSID already in intent or duplicate network_id"),
...
pub async fn create_device_intent(
    ...
    let ssid_conflict = sqlx::query_scalar!(
        r#"
        SELECT EXISTS (
            SELECT 1
            FROM device_network_intent dni
            ...
            WHERE dni.device_id = $1
              AND existing_n.ssid = new_n.ssid
    ...
    .await
    .map_err(|err| {
        error!("Failed to check SSID uniqueness: {err}");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = StatusCode::BAD_REQUEST, description = "No intent (version 0) or NULL password"),
        (status = StatusCode::NOT_FOUND, description = "Device not found"),
